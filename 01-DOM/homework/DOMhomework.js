...
let creando = document.getElementById("createdBy");
// creando.innerText += ' M. Soruco'
creando.append(" M. Soruco");
...
function buildToDos(toDos) {
  // Tu código acá:
  // map y filter devuelven nuevo arreglo
  // let arr = toDos.map(function(toDo, i){
  //   return buildToDo(toDo, i);
  // })
  //ESTO LO HACE DE FORMA EXPLICITA

  var arr = toDos.map(buildToDo); // LO HACE DE FORMA IMPLICITA
  return arr;
}
...
function displayToDos() {
  // Tu código acá:
  var toDoContainer = document.querySelector("#toDoContainer"); // <div id=toDoContrainer> </div>
  toDoContainer.innerHTML = "";
  let build = buildToDos(toDoItems);
  for (let i = 0; i < build.length; i++) {
    toDoContainer.appendChild(build[i]);
  }
  // tambien para recorrer se puede usar
...
function addToDo() {
  ...
  inp.value = ""; // me queda el input vacio -> UX -> Experiencia de usuario
  // Consigna 4
  displayToDos();
}
